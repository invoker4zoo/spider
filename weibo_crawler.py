...
class WeiBoScraper(object):
    ...
    def _get_weibo_detail_comment(self):
        ...
        with open(weibo_comments_save_path, 'w+') as f:
            for i, url in enumerate(self.weibo_detail_urls):
                ...
                print(u'微博内容： {}'.format(self.weibo_content[i]))
                print(u'接下来是下面的评论：\n\n')
    ...
    def write_text(self):
        try:
            ...
            print(u'微博写入文件完毕，保存路径%s' % file_path)
...
def main():
    ...
    print(u'最新一条微博为：', wb.weibo_content[0])
    print(u'最新一条微博获得的点赞数：', wb.num_zan[0])
    print(u'最新一条微博获得的转发数：', str(wb.num_forwarding[0]))
    print(u'最新一条微博获得的评论数：', str(wb.num_comment[0]))
    wb.write_text()
